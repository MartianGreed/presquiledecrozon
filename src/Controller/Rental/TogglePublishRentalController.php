    ...
    public function __invoke(Request $request, #[MapEntity(id: 'id')] Rental $rental): Response
    {
        if (! $rental->isPublishable() && ! $rental->isPublished()) {
            return new JsonResponse([
                'message' => 'Vous ne pouvez pas publier la location. Vous devez avoir un abonnement valide pour pouvoir le faire.',
            ], 403);
        }
        if ($rental->isPublishable() && ! $rental->isPublished()) {
            ...
            return new JsonResponse([
                'message' => 'Rental has successfully been published',
            ]);
        }
